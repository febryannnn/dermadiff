#!/usr/bin/env python3
"""
DermaDiff — Phase 3 (DoRA): PanDerm Classifier Training
=========================================================

Trains the PanDerm ViT-Large classifier on HAM10000 augmented with synthetic
images from the DoRA Phase 2. Identical to the LoRA Phase 3 classifier script
in every respect — the only difference is that --synthetic_dir should point
at the output of the DoRA generation (dora_sdxl_*.jpg files) instead of the
LoRA one (sdxl_lora_*.jpg files). The classifier training itself has zero
awareness of whether the synthetics came from LoRA or DoRA. Builds the training CSV in PanDerm's expected format,
...
def main():
    parser = argparse.ArgumentParser(
        description="Train PanDerm classifier on HAM10000 + SDXL DoRA synthetics"
    )
    ...
    parser.add_argument("--exp_name", default="DermaDiff_C3_DoRA",
